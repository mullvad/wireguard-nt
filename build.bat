...
signtool sign /tr %TIMESTAMP_SERVER% /td sha256 /fd sha256 /sha1 "%1" /v /ac %ROOT%%CROSSCERT% %ROOT%Release\amd64\driver\wireguard.sys

IF %ERRORLEVEL% NEQ 0 goto ERROR

:: Re-generate catalog file now that driver binary has changed

del %ROOT%Release\amd64\driver\wireguard.cat
...
signtool sign /tr %TIMESTAMP_SERVER% /td sha256 /fd sha256 /sha1 "%1" /v /ac %ROOT%%CROSSCERT% %ROOT%Release\amd64\driver\wireguard.cat
